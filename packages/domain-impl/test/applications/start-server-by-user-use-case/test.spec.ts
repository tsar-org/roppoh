import { StartServerByUserUseCaseImpl } from "@domain-impl/applications";
...
import {
  ...
  composeOneStatus200Error,
  composeOneStatus404,
  composeOneStatus500,
} from "./handler.msw";

describe("StartServerByUserUseCaseImpl", () => {
  ...
  async function di() {
    ...
    container.bind(StartServerByUserUseCaseImpl).toSelf();

    return await container.getAsync(StartServerByUserUseCaseImpl);
  }
  ...
  test("throws InvalidServerStateError when server status is not stopped", async () => {
    // arrange
    server.use(composeOneStatus200Error);
    const useCase = await di();

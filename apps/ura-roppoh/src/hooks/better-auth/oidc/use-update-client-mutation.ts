...
export const useUpdateClientMutation = (args: Args) => {
  const query = useQueryClient();

  const mutationFn = async (
    params: Parameters<NonNullable<(typeof auth)["oauth2"]["updateClient"]>>[0],
  ) => {
    const { data, error } = await auth.oauth2.updateClient(params);

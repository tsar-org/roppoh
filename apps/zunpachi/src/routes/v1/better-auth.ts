...
import { type HonoEnv, ServiceIdentifier } from "@/middlewares/dependency-injection";
...
export const betterAuthRoute = new Hono<HonoEnv>().all("", async (c) => {
  const betterAuth = await c.env.container.getAsync<BetterAuth>(ServiceIdentifier.BETTER_AUTH);

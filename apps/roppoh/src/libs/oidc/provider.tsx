...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children, issuer, clientId }) => {
  ...
  const [accessToken, setAccessTokenState] = useState<AuthContextType["accessToken"]>();
  ...
  const setAccessToken = (token?: string) => {
    ...
    if (token) {
      ...
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ ...stored, accessToken: token }));
    }
  };
  ...
  const setIdToken = (token?: string) => {
    ...
    if (token) {
      const stored = JSON.parse(localStorage.getItem(STORAGE_KEY) || "{}");
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ ...stored, idToken: token }));
    }
  };
  ...
  return (
    <AuthContext.Provider
      value={{ as, client, accessToken, setAccessToken, idToken, setIdToken, user, setUser }}
    >
      {children}
    </AuthContext.Provider>
  );
